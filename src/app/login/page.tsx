...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (message === 'link_expired') {
      setError('⚠️ Your confirmation link has expired. Please sign in or request a new password reset — links are valid for 24 hours.')
    ...
    if (error) {
      ...
      try {
        await fetch('/api/send-welcome', {
          ...
          body: JSON.stringify({ email: form.email, fullName: form.full_name, role: form.role, country: form.country }),
        })
      } catch (e) { console.error('Welcome email failed:', e) }
      setSuccess('🎉 Account created! Check your email to confirm your account. A welcome email is on its way!')
    }
    setLoading(false)
  }
        ...
        {tab === 'forgot' ? (
          <>
            ...
            {!success ? (
              ...
              <button onClick={() => switchTab('signin')} className="btn-primary w-full justify-center py-3">
                Back to Sign In →
              </button>
            )}
          </>
